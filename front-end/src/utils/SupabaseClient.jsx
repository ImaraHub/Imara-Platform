  ...
  export const getAllProjectContributors = async (projectId) => {
    try {
        const { data: contributors, error: contributorsError } = await supabase
            .from('idea_contributors')
            .select('*, user:user_id(email, username)') // Select all contributor fields and user info
        ...
        // The result already includes nested user data if the join was successful
        // We might need to flatten or map based on the exact structure returned by Supabase
        // For now, assuming the user data is directly available as 'user' on each contributor object
        const contributorsWithUserInfo = contributors.map(contributor => {
            // Supabase often returns related data as an array if multiple matches are possible
            // but with .single() or direct foreign key joins, it might be an object.
            // Let's assume the user data is directly under `contributor.user`
